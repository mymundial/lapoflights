(() => {
  ...
  function navIcon(id){
    const icons={
      ...
      sleigh:`<span class="nav-sleigh-icon-wrap" aria-hidden="true"><img class="nav-sleigh-icon nav-sleigh-off" src="./assets/nav-sleigh-unselected.png" alt=""><img class="nav-sleigh-icon nav-sleigh-on" src="./assets/nav-sleigh-selected.png" alt=""></span>`
    };
    return icons[id]||'';
  }
  ...
  function topBar(){ return `<header class="topbar">${brand()}<img class="mission-control-logo" src="./assets/mission-control-logo.png" alt="Mission Control"><div class="masthead-strap">Santa's Sleigh Recovery</div></header>`; }
  ...
  function renderLaunch(){
    if(state.bootDone===false) return `<section class="hero"><div class="hero-content"><img class="hero-logo hero-logo-stacked" src="./assets/silverstone-logo-landing.png" alt="Silverstone"><img class="hero-mission-logo" src="./assets/mission-control-logo.png" alt="Mission Control"><div class="strap">Santa's Sleigh Recovery</div><div class="hero-actions"><button class="btn primary" data-onboard="brief">Start Mission</button><button class="btn secondary" data-onboard="demo">Quick Demo</button></div></div></section>`;
  ...
  function missionHeader(cp){
    ...
    if(cp.type==='diagnostics'){
      return `<div class="mission-head diagnostics-head"><div class="meta"><div class="kicker">${label}</div><button class="linkbtn" data-exit-mission>Exit Mission</button></div><div class="diagnostics-brand"><img src="./assets/audi-rings.png" alt="Audi"></div><h1>Performance Scan</h1><p class="support-copy">${missionInstruction(cp.type)}</p></div>`;
    }
  ...
  function renderMission(id){
    ...
    return `<div class="mission-shell">${topBar()}${missionHeader(cp)}<div class="mission-content" id="missionContent">${missionBody(cp)}</div></div>`;
  }
  ...
  function diagnosticsBody(){
    const sensors=[
      ...
      {name:'Traction',key:'traction',viz:`<svg viewBox="0 0 120 70" role="presentation"><path class="traction-car" d="M49 13h22l8 12v20l-8 12H49l-8-12V25l8-12Z"/><rect class="patch p1" x="29" y="17" width="14" height="13" rx="5"/><rect class="patch p2" x="77" y="17" width="14" height="13" rx="5"/><rect class="patch p3" x="29" y="40" width="14" height="13" rx="5"/><rect class="patch p4" x="77" y="40" width="14" height="13" rx="5"/><path class="traction-drive" d="M60 18v34"/></svg>`},
      {name:'Recovery',key:'recovery',viz:'<i class="recovery-ring"></i><b></b>'}
    ];
    return `<div class="mission-instrument panel diagnostics-panel"><div class="sensor-grid diagnostics-grid">${sensors.map((x,i)=>`<button class="sensor sensor-${x.key}" data-sensor="${i}" data-diagnostic="${x.key}"><div class="num">0${i+1}</div><div class="name">${x.name}</div><div class="sensor-viz viz-${x.key}" aria-hidden="true">${x.viz}</div><div class="state">Ready to scan</div></button>`).join('')}</div></div><button class="btn primary wide" id="diagComplete" disabled>Confirm Performance Data</button>`;
  }
  ...
  function bindLapland(){
    ...
    const checks=[{id:'power'},{state:'elfUnlocked'},{id:'spirit'},{id:'comet'},{id:'jingle'},{id:'lando'},{id:'aurora'}];
    btn.onclick=()=>{
      ...
      rows.forEach((r,i)=>setTimeout(()=>{
        const ready=checks[i].state==='elfUnlocked'?state.elfUnlocked:state.completed.includes(checks[i].id);
        r.classList.toggle('ok',ready); r.classList.toggle('attention',!ready);
        r.children[1].textContent=ready?'Verified':'Required';
        if(!ready) missing.push(checks[i].id);
